...
import online.daliyq.api.adapter.LocalDateAdapter
import online.daliyq.api.converter.LocalDateConverterFactory
import online.daliyq.api.response.*
...
interface ApiService {
    companion object {
        ...
        private fun okHttpClient(context: Context): OkHttpClient {
            ...
            return builder.connectTimeout(3, TimeUnit.SECONDS).writeTimeout(10, TimeUnit.SECONDS)
                .readTimeout(10, TimeUnit.SECONDS).cache(cache).addInterceptor(AuthInterceptor())
                .authenticator(TokenRefreshAuthenticator()).addInterceptor(logging)
                .addInterceptor(EndpointLoggingInterceptor("AppInterceptor", "answers"))
                .addNetworkInterceptor(EndpointLoggingInterceptor("NetworkInterceptor", "answers")).build()
        }
        ...
        private fun create(context: Context): ApiService {
            ...
            return Retrofit.Builder().addConverterFactory(GsonConverterFactory.create(gson))
                .addConverterFactory(LocalDateConverterFactory()).baseUrl("http://10.0.2.2:5000")
                .client(okHttpClient(context)).build().create(ApiService::class.java)
        }
    ...
    suspend fun getQuestions(
        @Query("from_date") fromDate: LocalDate, @Query("page_size") pageSize: Int
    ...
    suspend fun writeAnswer(
        @Path("qid") qid: LocalDate, @Field("text") text: String? = null, @Field("photo") photo: String? = null
    ): Response<Answer>
    ...
    suspend fun deleteAnswer(
        @Path("qid") qid: LocalDate, @Path("uid") uid: String? = AuthManager.uid
    ): Response<Unit>
