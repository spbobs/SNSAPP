...
class UserAnswerAdapter(context: Context) :
    ...
    object QuestionResponseDiffCallback : DiffUtil.ItemCallback<QuestionAndAnswer>() {
        override fun areItemsTheSame(oldItem: QuestionAndAnswer, newItem: QuestionAndAnswer): Boolean {
            return oldItem.question.id == newItem.question.id
        }

        override fun areContentsTheSame(oldItem: QuestionAndAnswer, newItem: QuestionAndAnswer): Boolean {
